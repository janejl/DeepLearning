...
class PretrainedWord2Vec(object):
    ...
    def _load_word2vec(self, glove_path):
        ...
        with open(glove_path, 'r') as f:
            for line in f.readlines():
                data = line.split(' ')
                embeddings[data[0]] = np.asarray(data[1:], dtype=np.float32)
        ### END YOUR CODE
        return embeddings
    ...
    def get_vector(self, word):
        ...
        """
        ### YOUR CODE HERE
        vector = self.embeddings[word] if word in self.embeddings else oov_vector
        ### END YOUR CODE
        return vector
    ...
    def word_similarity(self, word1, word2):
        ...
        sim = v1.T.dot(v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))
        ### END YOUR CODE
        return sim
    ...
    def most_similar(self, word, num_candidates=3):
        ...
        """
        ### YOUR CODE HERE
        a = np.array(list(map(lambda x: self.word_similarity(x, word), self.embeddings.keys()))).argsort()
        indexes = a[-(num_candidates+1):-1][::-1] # get last n sorted index (except the last one) in reversed order (descending)
        words = list(self.embeddings.keys()) 
        return [words[i] for i in indexes] # get similar words by index
        ### END YOUR CODE
...
if __name__ == '__main__':
    ...
    # Init from file
    model = PretrainedWord2Vec("/userhome/34/ljiang/deep_learning/A1/Part_2/datasets/glove.6B.50d.txt")
    ...
    print("Top 3 similar words to `queen':")
