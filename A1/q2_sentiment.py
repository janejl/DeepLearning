...
def create_model_and_compile(output_dim=1, hidden_dim=50, learning_rate=0.0001):
    ...
    model.add(tf.keras.layers.Dense(hidden_dim, activation=tf.nn.relu))
...
def train(n_epochs=20, batch_size=32, learning_rate=0.001):
    ...
    processor = DataProcessor(glove_path="/userhome/34/ljiang/deep_learning/A1/Part_2/datasets/glove.6B.50d.txt", debug=False)
