...
def forward_propagation(X, layer_params):
    ...
    ### YOUR CODE HERE (~4 lines)
    Z1 = layer_params['W1'].dot(X) + layer_params['b1']
    A1 = sigmoid(Z1)
    Z2 = layer_params['W2'].dot(A1) + layer_params['b2']
    A2 = sigmoid(Z2)
    ### END YOUR CODE
...
def backward_propagation(Y, Y_hat, cache, layer_params):
    ...
    dZ1 = sigmoid_grad(Z1) * W2.T.dot(dZ2)
    dW1 = 1 / m * dZ1.dot(X.T)
...
def parameter_update(grads, layer_params, learning_rate):
    ...
    for k in ['W1', 'W2', 'b1', 'b2']:
        layer_params[k] -= learning_rate * grads[k]
    ### END YOUR CODE
    return layer_params
...
def cost_fn(Y_hat, Y):
    ...
    """
    ### YOUR CODE HERE
    y, y_hat, m = Y[0], Y_hat[0], Y.shape[1]
    loss = -1 * sum(y * np.log(y_hat) + (1 - y) * np.log(1 - y_hat))
    ### END YOUR CODE
    return loss / m
